...
import { House, Cpu, ShoppingBag, GitBranch, LayoutTemplate, MessageCircle, Database } from 'lucide-react'
    ...
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] pointer-events-none hidden lg:flex justify-center pt-6">
        <motion.div
          ...
          className={`pointer-events-auto flex items-center gap-1 p-1.5 rounded-full border border-[#222222] bg-[#111111]/85 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] transition-all duration-300 ${isScrolled ? 'scale-[0.97] bg-[#111111]/95' : ''}`}
        >
          ...
          <Link href="/contato" className="px-5 py-2.5 rounded-full bg-brand-primary text-[#0A0A0A] text-[11px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-[0_0_24px_var(--color-brand-glow)] whitespace-nowrap hover:bg-brand-hover">
            Contato
          </Link>
        </motion.div>
      </nav>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[100] lg:hidden pointer-events-none" style={{ paddingBottom: 'max(1.25rem, env(safe-area-inset-bottom))' }}>
        <div className="px-4 pointer-events-auto">
          <motion.div
            ...
            className="flex items-center justify-around p-2 rounded-[2rem] border border-[#222222] bg-[#111111]/95 backdrop-blur-2xl shadow-[0_-8px_32px_rgba(0,0,0,0.6)]"
          >
            {navLinks.map((link) => {
              const isActive = pathname === link.path || (link.path !== '/' && pathname.startsWith(link.path));
              const Icon = link.icon;
            ...
            <Link href="/contato" className="relative flex flex-col items-center justify-center gap-1 py-1.5 px-2.5 min-w-[3rem] rounded-xl transition-all active:scale-90" style={{ WebkitTapHighlightColor: 'transparent' }}>
              <div className="relative z-10 p-1.5 rounded-lg bg-brand-primary text-[#0A0A0A] shadow-[0_0_16px_var(--color-brand-glow)]">
